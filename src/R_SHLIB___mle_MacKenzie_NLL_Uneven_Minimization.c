...
void mle_MacKenzie_NLL_Uneven_Minimization_DRIVER ( int No_of_SPECIES, char ** Species_Tag, double *** Presence, int * No_of_SITES, double ** Time_Vector, int * No_of_TIMES, double MISSING_VALUE_FLAG, double Colonization_Rate, double * C_Range, double Extinction_Rate, double * E_Range, double Detectability, double * D_Range, double Phi_Time_0, double * P_Range, int * No_of_PARAMETERS, int * No_of_PARAMETERS_MAX, int * Index, int * Discretization, double * Tolerance, int * No_of_ITERATIONS, int * Verbose, int * Minimization, double ** Results );

void R_SHLIB___mle_MacKenzie_NLL_Uneven_Minimization ( int * pNo_of_SPECIES, char ** Species_Tag, double * R_Presence, int * No_of_SITES, double * R_Time_Vector, int * No_of_TIMES, double * MISSING_VALUE_FLAG, double * Colonization_Rate, double * C_Range, double * Extinction_Rate, double * E_Range, double * Detectability, double * D_Range, double * Phi_Time_0, double * P_Range, int * No_of_PARAMETERS, int * No_of_PARAMETERS_MAX, int * Index, int * Discretization, double * Tolerance, int * No_of_ITERATIONS, int * Verbose, int * Minimization, double * R_Results )
{
  /* Input parameters:
  ...
  */
  ...
  /* No_of_SITES will change down the code. We keep a copy of this object in the
     No_of_SITES_BIS object in order to be able to free exactly the same amount of
     memory we have allocated!!!
  */
  int * No_of_SITES_BIS = (int *)calloc( No_of_SPECIES, sizeof(int) );
  for( i=0; i<No_of_SPECIES; i++ )
    No_of_SITES_BIS[i] = No_of_SITES[i];

  double ** Results = (double **)calloc( No_of_SPECIES, sizeof(double *) );
  for( i=0; i<No_of_SPECIES; i++ ){
    Results[i] = (double *)calloc( 5, sizeof(double) );
  }
  ...
  mle_MacKenzie_NLL_Uneven_Minimization_DRIVER (No_of_SPECIES, Species_Tag, Presence, No_of_SITES, Time_Vector, No_of_TIMES, (* MISSING_VALUE_FLAG), (* Colonization_Rate), C_Range, (* Extinction_Rate), E_Range, (* Detectability), D_Range, (* Phi_Time_0), P_Range, No_of_PARAMETERS, No_of_PARAMETERS_MAX, Index, Discretization, Tolerance, No_of_ITERATIONS, Verbose, Minimization, Results);
  ...
  for( i=0; i<No_of_SPECIES; i++ ){
    free(Time_Vector[i]);
    for (j=0; j<No_of_SITES_BIS[i]; j++) free( Presence[i][j] );
    free(Presence[i]);
  }
  free( Presence ); free( Time_Vector);
  free( No_of_SITES_BIS );
  /*   END: ------------------------------------------------------------
   */
}

void mle_MacKenzie_NLL_Uneven_Minimization_DRIVER (int No_of_SPECIES, char ** Species_Tag, double *** Presence, int * No_of_SITES, double ** Time_Vector, int * No_of_TIMES, double MISSING_VALUE_FLAG, double Colonization_Rate, double * C_Range, double Extinction_Rate, double * E_Range, double Detect_Probability, double * D_Range, double Phi_Time_0, double * P_Range, int * No_of_PARAMETERS, int * No_of_PARAMETERS_MAX, int * Index, int * Discretization, double * Tolerance, int * No_of_ITERATIONS, int * Verbose, int * Minimization, double ** Results )
{
  int i,j,k,l,s,  n;
  ...
  */
  ...
  int * Total_No_of_Transects  = (int *)calloc( No_of_SPECIES, sizeof(int) );
  for(i=0; i<No_of_SPECIES; i++) Total_No_of_Transects[i] = No_of_TIMES[i];

  double * Dummy;
  int    Dummy_No_of_TIMES;
  for(i=0; i<No_of_SPECIES; i++) {
    Dummy = (double *)calloc( No_of_TIMES[i], sizeof(double) );
    Dummy_No_of_TIMES = No_of_TIMES[i];
  ...
  for( i=0; i<No_of_SPECIES; i++ ) {
    ...
    SP_Matrix_Data_Alloc( No_of_SITES[i], No_of_TIMES[i], Total_No_of_Transects[i],
			  Data[i]);
    SP_Matrix_Data_Setup( No_of_SITES[i], No_of_TIMES[i], Total_No_of_Transects[i],
			  Data[i], Presence[i],
			  Time_Vector[i], Sp_Time[i], Sp_No_of_Times[i],
			  Transects[i], Species_Tag[i] );
  }

  IO_Filtering_Out_Missing_Values ( No_of_SPECIES, Presence, No_of_SITES,
  ...
  */
  ...
  for( i=0; i<No_of_SPECIES; i++ ){
    ...
    */
    ...
    mle_MacKenzie_Uneven_Matrix_R_SHLIB( Presence[i], No_of_SITES[i], Total_No_of_Transects[i], Time_Vector[i], Transects[i], No_of_TIMES[i], Sp_Time[i], Sp_No_of_Times[i], Sp_Transect[i], Sp_No_of_Transects[i], &Colonization[i], C_Range, &Extinction[i], E_Range, &Detectability[i], D_Range, &Phi_T_0[i], P_Range, No_of_PARAMETERS, No_of_PARAMETERS_MAX, Index, Discretization, Tolerance, No_of_ITERATIONS, Verbose, Minimization, &NLL_Value[i] );
