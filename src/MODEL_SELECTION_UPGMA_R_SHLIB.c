	...
	double ** Results )
{
		...
		*/
		...
		{ Rprintf("Number of Sampling Times does not match: program aborted"); error(0,0,"Program aborted"); }
	  ...
	  /*     E N D: Reserving memory ---------------------- */
		...
		*/
		for(i=0; i<No_of_SPECIES; i++) { /* i-th Partitition:
			Since partition are made by a upgma algorithm,
			...
			for(j=0; j<G[i]; j++) {
				...
				for (k=0; k<R[i][j]; k++)
				Sp_Time_Vector[j][k] = (double *)calloc(General_No_of_SAMPLING_TIMES, sizeof(double ));
			}
			...
			*/
			if( (*Verbose) == 1 ) Rprintf(" %dth Partition is made up by %d subsets: ", i, G[i]);
			for(j=0; j<G[i]; j++) {
				if( (*Verbose) == 1 ) Rprintf("{ ");
				for(k=0; k<K[i][j]; k++)
			...
			*/
			...
			for( j=0; j<G[i]; j++ ) {
			  ...
			  if( N00 + N01 + N10 + N11 != No_of_TRANSITIONS )
				{ Rprintf("Number of Transititions do not sum up"); error(0,0,"Program aborted"); }
