					     ...
					     double ** Results )
{
  ...
  */
  ...
  for(i=0; i<No_of_SPECIES; i++) { /* i-th Partitition:
    ...
    */
    for( j=0; j<G[i]; j++ ){
      for (k=0; k<R_que_R[i][j]; k++) free(Sp_Time_Vector[j][k]);
    ...
    for( j=0; j<G[i]; j++ ){
      for (k=0; k<R_que_R[i][j]; k++)  free(Sp_Transect[j][k]);
      free(Sp_Transect[j]);
    }
    free(Sp_Transect);
  ...
  free(NLL); free(COL); free(EXT); free(DTC); free(P_0); free(R); free(R_que_R);
  free(MODEL_NLL);
  free(MODEL_AIC);
  free(MODEL_AIC_c);
